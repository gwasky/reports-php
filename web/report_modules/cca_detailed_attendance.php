...
function generate_detailed_report($from,$to,$shift,$supervisors,$teams){
	...
	$query = "SELECT 
		...
		cc_attendance.shift as shift,
  		...
  		cc_attendance_cstm.checkin_time_c as checkin_time,
		...
		cc_attendance.shift_completion_status as completion,
		cc_attendance.reasons_for_not_completing as reason_completion
		FROM
  		cc_teams
	...
	if($teams){
		...
		foreach($attendance_records as $row)
		{
			...
			$report[data][$row[agent]][$row[attendance_date]][team_name] = $row[team_name];
			$report[data][$row[agent]][$row[attendance_date]][checkin_time] = $row[checkin_time];
			$report[data][$row[agent]][$row[attendance_date]][checkout_time] = $row[checkout_time];
			$report[data][$row[agent]][$row[attendance_date]][completion] = $row[completion];
			$report[data][$row[agent]][$row[attendance_date]][reason_completion] = $row[reason_completion];
			
		}
		//var_dump($report);
		return display_attendance_detailed_hmtl($report,$dateRangeArray);}else { return dislay_blanks($report[daterange]);}
}

function display_attendance_detailed_hmtl($report){
	$html = '
	...
	{
		$html .='
			<th colspan="8" align="center">'.$date.'</th>
		';
	}
	...
	foreach($report[data] as $agent=>$agent_data)
	{
		...
		foreach($report[daterange] as $date)
		{
			$html .= '
				...
				<td class="text_values">'.$agent_data[$date][team_name].'</td>
				<td class="text_values">'.$agent_data[$date][checkin_time].'</td>
				<td class="text_values">'.$agent_data[$date][checkout_time].'</td>
				<td class="text_values">'.$agent_data[$date][completion].'</td>
				<td class="text_values">'.$agent_data[$date][reason_completion].'</td>
			';
		}
		$html .= '
		</tr>
