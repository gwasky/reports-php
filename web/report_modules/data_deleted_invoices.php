...
function generate_deleted_invoices($from,$to){
	...
	//$i = 0;
	foreach($data as $row){
		array_push($unserialised,unserialize($row[details]));
		//++$i;
	}
	...
	if($from){
		...
		$Y = date('Y'); $m = date('m') - 1;
		array_push($conditions,array('billing_date','>=',$Y."-".$m."-01"));
	}
	if($to){
		array_push($conditions,array('billing_date','<=',$to));
	}else{
		array_push($conditions,array('billing_date','<=',date('Y-m-d')));
	}
	
	array_push($conditions,array('deleted','=',1));
	...
	return display_deleted_invoice_report($report,$from,$to);

}

function display_deleted_invoice_report($report,$from,$to){
