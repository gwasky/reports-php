<?php
function generate_monthly_csat_t($to){
...
function get_number_of_cases_created_t($to){
	if($to == '' ) { $to = date('Y-m-d',strtotime("-1 days")).' 23:59:59'; } else { $to .= " 23:59:59";}
	$from = date('Y-m-',strtotime("-1 days")).'01 00:00:00';
...
function get_wraups_and_repeats_totals_t($to){
	if($to == '' ) { $to = date('Y-m-d',strtotime("-1 days")).' 23:59:59'; } else { $to .= " 23:59:59";}
	$from = date('Y-m-',strtotime("-1 days")).'01 00:00:00';
...
function get_cc_quality_score_t($to){
	if($to == '' ) { $to = date('Y-m-d',strtotime("-1 days")).' 23:59:59'; } else { $to .= " 23:59:59";}
	$from = date('Y-m-',strtotime("-1 days")).'01 00:00:00';
	...
	//echo nl2br($query);
	//echo '<pre>'.print_r($rows,true).'</pre>'.$rows['overallscore'];

	$returnscore = number_format($rows['overallscore'],2).'%';
	return $returnscore;
}

function get_ccprojections_t($to){
...
function generate_cc_flash_report_t($date_input){
	...
	if($myquerys->single($check_query,'')){
		...
		$report[mtd]['CSAT'] = number_format(generate_monthly_csat_t($date_input),2).'%';
		$wrapup_totals = get_wraups_and_repeats_totals_t($date_input);
		$report[mtd]['FIRSTCALLRES'] = number_format(((($wrapup_totals['total_wp'] - $wrapup_totals['repeat_wp']) - get_number_of_cases_created_t($date_input))/$wrapup_totals['total_wp']*100),2).'%';
		//print get_cc_quality_score($date_input);
		$report[mtd]['QLT'] = get_cc_quality_score_t($date_input);
		
		$number_of_days_mtd = ((((strtotime($date_input) - strtotime(date('Y-m',strtotime($date_input)).'-01'))/60)/60)/24)+1;
		$report[mtd]['projected_calls'] = get_ccprojections_t($date_input);
		...
		$calls_percentage_perday = ($report[mtd]['Calls_Received_calls_perday']/number_format($report[mtd]['projected_calls_perday'],0))*100;
		
		if($calls_percentage > 100){
			$report[mtd]['calls_percentage'] = '<span style="color:red; font-weight:bold;">'.number_format($calls_percentage,2).'%</span>';
		}else{
			$report[mtd]['calls_percentage'] = '<span style="color:green; font-weight:bold;">'.number_format($calls_percentage,2).'%</span>';
		}
		
		if($calls_percentage_perday > 100){
			$report[mtd]['calls_percentage_perday'] = '<span style="color:red; font-weight:bold;">'.number_format($calls_percentage_perday,2).'%</span>';
		}else{
			$report[mtd]['calls_percentage_perday'] = '<span style="color:green; font-weight:bold;">'.number_format($calls_percentage_perday,2).'%</span>';
		}
		
		//$report[mtd]['Calls Received']/$number_of_days_mtd;
		foreach($report[data]['Overall Calls Recieved'] as $dates => $calls_received){
			$calls_percentage_day = ($calls_received/($report[mtd]['projected_calls']/date('t',strtotime($date_input))))*100;
			$report[data]['projected_calls_perday'][$dates] = $report[mtd]['projected_calls_perday'];
			if($calls_percentage_day > 100){
				$report[mtd]['calls_percentage_day'][$dates] = '<span style="color:red; font-weight:bold;">'.number_format($calls_percentage_day ,2).'%</span>';
			}else{
				$report[mtd]['calls_percentage_day'][$dates] = '<span style="color:green; font-weight:bold;">'.number_format($calls_percentage_day ,2).'%</span>';
			}
		}
...
function get_color_code_t($val, $con_target, $condition){
$val = (int) rtrim($val,'%');
if($condition == '=='){
...
function display_cc_flash_report_t($report){
...
.value{
	...
	foreach($dates as $date){
		$html .= '<td scope="col" class="top_th">'.$date.'</td>';
	}
	...
	$html .= '</tr>
				<tr class="row" bgcolor="#CCCCCC"><th><strong>Actual No. of Calls:</strong></th>';
	foreach($dates as $date){
		$html .= '<td class="value">'.number_format($report[data]['Overall Calls Recieved'][$date],0).'</td>';
	}
	$html .= '</tr>
				<tr><th><strong>Percentage:</strong></th>';
	foreach($dates as $date){
