...
function generate_yesterday_top_network_wrapups(){
	...
	$query = "
		(
			...
			WHERE
				reportsphonecalls.wrapupsubcat IN ('Network','Network Complaints','Network Coverage','Network quality') AND
	...
	return generate_network_calls_top_wrapups_for_last_30_days($top_wrapups_list);	}

function generate_network_calls_top_wrapups_for_last_30_days($top_wrapups_list){
	$myquery = new custom_query();
	$my_graph = new dbgraph();
	...
	/* For Complaints-(Negative Feedback) */
	if(!empty($data['Negative Feedback'])){
		...
		$graph_detail['title']='Network Wrapups (Complaints)';
		...
		$report['empty']['complaints'] = "No Compliants Were Recorded For The Specified Period";
	}
	...
	if(!empty($data['Inquiry'])){
		...
		$graph_detail['title']='Network Wrapups (Inquiries)';
		$graph_detail['display_title']=false;
		$graph_detail['legend']=true;
		$graph_detail['line_graph']=true;
		$graph_detail['bar_graph']=false;
		...
		$my_graph->graph($title = $graph_detail['title'], "30 days before ".$date, $graph_detail);
		custom_query::select_db('graphing');
		$report['graph_id']['inquiry'] = $my_graph->Save();	
	}
	else{
		$report['empty']['inquiry'] = "No Inquiries Were Recorded For The Specified Period";
	}
	
	return display_network_wrapups_report($report, $date);}

function display_network_wrapups_report($report, $date){	
	$html = '
		<table border="0" cellpadding="1" cellspacing="0" style="float:left">
			<tr>
				<th colspan="3">NETWORK WRAPUPS FOR THE LAST 30 DAYS</th>
			</tr>
			<tr>
				<th colspan="3">TOP TWO INQUIRIES/SERVICE (RESTORATION) REQUESTS FOR '.$date.'</th>
			</tr>';
	if(!isset($report['empty']['inquiry'])){		
		$html .= '		
			<tr>
		...
		$html .= '
			<tr>
				<td colspan="3">'.$report['empty']['inquiry'].'</td>
			</tr>';
	}
	$html .= '
			<tr>
	...
	if(!isset($report['empty']['complaints'])){		
		$html .= '		
			<tr>
				<th class="text_values">#</th>
				<th class="text_values">SUBJECT</th>
		...
		$html .= '
			<tr>
				<td colspan="3">'.$report['empty']['complaints'].'</td>
			</tr>';
	}
	$html .= '
		</table>
