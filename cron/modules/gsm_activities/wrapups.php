...
//TO COLLECT DATA FROM CCBA02.REPORTSCRM

function generate_gsm_wrapup_summary($date){
	...
	$query = "
		select
			(SELECT count(*) as Number FROM reportsphonecalls WHERE reportsphonecalls.createdon between '".$date." 00:00:00' and '".$date." 23:59:59') as yesterday_wrapups,
			(SELECT count(*) as Number FROM reportsphonecalls WHERE reportsphonecalls.createdon between date_sub('".$date." 00:00:00', interval 1 month) and date_sub('".$date." 23:59:59', interval 1 month)) as last_month_wrapups,
			(SELECT count(*) as Number FROM reportsphonecalls WHERE reportsphonecalls.createdon between '".substr($date,0,7)."-01 00:00:00' and '".$date." 23:59:59') as months_wrapups,
			(SELECT count(*) as Number FROM reportsphonecalls WHERE reportsphonecalls.createdon between '".substr($date,0,4)."-01-01 00:00:00' and '".$date." 23:59:59') as years_wrapups
	";
	
	echo date('Y-m-d H:i:s')." : [".$date."] Generating wrap up summary - Base information ... \n";
	...
	wrapup_stats_bytype_add_baseinfo($wrapup[bases],$date);
	...
	echo date('Y-m-d H:i:s')." : [".$date."] Generating wrap up summary - Inquiries ... \n";
	
	//Inquiries
	$wrapup[data_sets][inquiries][day] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bytype($date,$period='day',$type='Inquiry'),
												$total_wrapups = $wrapup[bases][yesterday_wrapups]
											);
	
	$wrapup[data_sets][inquiries][month] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bytype($date,$period='month',$type='Inquiry'),
												//$raw_stats = generate_wrapup_stats_bytype($date,$period='day',$type='Inquiry'),
												$total_wrapups = $wrapup[bases][last_month_wrapups]
											);
	
	$wrapup[data_sets][inquiries][year] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bytype($date,$period='year',$type='Inquiry'),
												//$raw_stats = generate_wrapup_stats_bytype($date,$period='day',$type='Inquiry'),
												$total_wrapups = $wrapup[bases][years_wrapups]
											);
	
	echo date('Y-m-d H:i:s')." : [".$date."] Generating wrap up summary - Service Requests ... \n";
	
	//Inquiries
	$wrapup[data_sets][service_requests][day] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bytype($date,$period='day',$type='Service Restoration Request'),
												$total_wrapups = $wrapup[bases][yesterday_wrapups]
											);
	
	$wrapup[data_sets][service_requests][month] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bytype($date,$period='month',$type='Service Restoration Request'),
												$total_wrapups = $wrapup[bases][last_month_wrapups]
											);
	
	$wrapup[data_sets][service_requests][year] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bytype($date,$period='year',$type='Service Restoration Request'),
												$total_wrapups = $wrapup[bases][years_wrapups]
											);

	echo date('Y-m-d H:i:s')." : [".$date."] Generating wrap up summary - Negative feed back ... \n";
	
	//Negative Feedback
	$wrapup[data_sets][negative_feedback][day] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bytype($date,$period='day',$type='Negative Feedback'),
												$total_wrapups = $wrapup[bases][yesterday_wrapups]
											);
	
	$wrapup[data_sets][negative_feedback][month] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bytype($date,$period='month',$type='Negative Feedback'),
												//$raw_stats = generate_wrapup_stats_bytype($date,$period='day',$type='Negative Feedback'),
												$total_wrapups = $wrapup[bases][last_month_wrapups]
											);
	
	$wrapup[data_sets][negative_feedback][year] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bytype($date,$period='year',$type='Negative Feedback'),
												//$raw_stats = generate_wrapup_stats_bytype($date,$period='day',$type='Negative Feedback'),
												$total_wrapups = $wrapup[bases][years_wrapups]
											);
	
	echo date('Y-m-d H:i:s')." : [".$date."] Generating wrap up summary - Language distribution ... \n";
	
	//Language distributiokn
	$wrapup[data_sets][language][day] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bylang($date,$period='day'),
												$total_wrapups = $wrapup[bases][yesterday_wrapups]
											);
	
	$wrapup[data_sets][language][month] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bylang($date,$period='month'),
												//$raw_stats = generate_wrapup_stats_bylang($date,$period='day'),
												$total_wrapups = $wrapup[bases][last_month_wrapups]
											);
	
	$wrapup[data_sets][language][year] = prep_wrapup_table_data(
												$raw_stats = generate_wrapup_stats_bylang($date,$period='year'),
												//$raw_stats = generate_wrapup_stats_bylang($date,$period='day'),
												$total_wrapups = $wrapup[bases][years_wrapups]
											);

	
	return show_gsm_wrapup_summary($wrapup);
}
...
function generate_wrapup_stats_bytype($date,$period,$type){
...
function generate_wrapup_stats_bylang($date,$period='day'){
...
function show_gsm_wrapup_summary($data){
...
//NOT NEEDED WITH NEW DB WRAP UP TYPING
function wrapup_stats_byservicetype_service_query($service_type){
	if($service_type != ''){ $query = " and subsubcategory.service_type = '".$service_type."' "; } return $query;
}

function wrapup_stats_bytype_add_baseinfo(&$base,$date,$period=''){
	...
	foreach($periods as $period){
		...
		/*$query = "
			...
			union
			SELECT
				subsubcategory.service_type as name,
				count(*) as num
			FROM
				reportsphonecalls
				LEFT OUTER JOIN subsubcategory ON (subsubcategory.subsubcategory=reportsphonecalls.subject) AND (subsubcategory.subcategory=reportsphonecalls.wrapupsubcat)
