...
function generate_ussd_summary($date){
	if($date==''){ $ivr = FALSE; return show_gsm_ivr_summary($ivr);}
	$myquery = new custom_query();
	
	custom_query::select_db('reportsussd');
	
	echo date('Y-m-d H:i:s')." : [".$date."] Generating USSD summary - Base information ... \n";
	
	$query = "
		SELECT count(*) AS sessions, sum(ussd_log.number_of_tx) AS hits FROM ussd_log WHERE ussd_log.start_time between '".$date." 00:00:00' AND '".$date." 23:59:59' AND ussd_log.service_code IN ('*157','*100','*144')
	";
	$result = $myquery->single($query);
	$ussd[bases][day][sessions] = $result[sessions];
	$ussd[bases][day][hits] = $result[hits];
	$query = "
		SELECT count(*) AS sessions, sum(ussd_log.number_of_tx) AS hits FROM ussd_log WHERE ussd_log.start_time between date_sub('".$date." 00:00:00', interval 1 month) and date_sub('".$date." 23:59:59', interval 1 month) AND ussd_log.service_code IN ('*157','*100','*144')
	";
	$result = $myquery->single($query);
	$ussd[bases][last_month_day][sessions] = $result[sessions];
	$ussd[bases][last_month_day][hits] = $result[hits];
	$query = "
		SELECT count(*) AS sessions, sum(ussd_log.number_of_tx) AS hits FROM ussd_log WHERE ussd_log.start_time between '".substr($date,0,7)."-01 00:00:00' AND '".$date." 23:59:59' AND ussd_log.service_code IN ('*157','*100','*144')
	";
	$result = $myquery->single($query);
	$ussd[bases][month][sessions] = $result[sessions];
	$ussd[bases][month][hits] = $result[hits];
	$query = "
		SELECT count(*) AS sessions, sum(ussd_log.number_of_tx) AS hits FROM ussd_log WHERE ussd_log.start_time between '".substr($date,0,4)."-01-01 00:00:00' AND '".$date." 23:59:59' AND ussd_log.service_code IN ('*157','*100','*144')
	";
	$result = $myquery->single($query);
	...
	echo date('Y-m-d H:i:s')." : [".$date."] Generating USSD summary - Period summaries ... \n";
	
	$ussd[data_sets][day] = prep_ussd_table_data(
								$raw_stats = generate_ussd_stats($date,$period='day'),
								$totals = $ussd[bases][day]
							);
	
	$ussd[data_sets][month] = prep_ussd_table_data(
								$raw_stats = generate_ussd_stats($date,$period='month'),
								$totals = $ussd[bases][month]
							);
	
	$ussd[data_sets][year] = prep_ussd_table_data(
								$raw_stats = generate_ussd_stats($date,$period='year'),
								$totals = $ussd[bases][year]
							);
	
	return show_ussd_summary($ussd);
}
...
function generate_ussd_stats($date,$period){
	$myquery = new custom_query();
		...
		FROM
			ussd_log
		WHERE
			ussd_log.service_code IN ('*157','*100','*144') AND
	";
...
function show_ussd_summary($data){
